...
void procesa_mensaje(char* topic, byte* payload, unsigned int length) { 
  ...
  if(String(topic)==topic_SUSCRIPCION) {
    int nuevoEstado = (mensaje[0]=='1') ? 1 : 0;
    xQueueSend(colaLed, &nuevoEstado, 0); // produce mensaje para la tarea que controla el Led
  }
}
...
//-----------------------------------------------------
// Consumidor de la cola → controla LED
void taskLed(void *pvParameters) {
  ...
  int valor;
  while(true) {
    if (xQueueReceive(colaLed, &valor, portMAX_DELAY) == pdTRUE) {
      Serial.println(DEBUG_STRING+"Tarea led recive mensaje: "+String(valor));
...
void setup() {
  ...
  // Arrancar las demás (esperarán al semáforo de MQTT para publicar)
